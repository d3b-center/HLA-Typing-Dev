...
def prepare(fastq1, fastq2, artindex, b2url, tag, outdir, phlatdir, ispe,nthreads=8):
    ...
    try:
        ...
        if ispe and fastq1 != '' and fastq2 != '':
            cmd = b2url + ' ' + ' --maxins 450 --no-mixed --no-discordant --no-unal -p ' + str(nthreads) + ' -D 20 -R 3 -N 0 -L 20 -i S,1,0.50 -x ' + artindex + '/ucsc.artHLA -1 ' + fastq1 + ' -2 ' + fastq2 + ' |grep -P "\\t(A\\*|B\\*|C\\*|DQA1\\*|DQB1\\*|DRB1\\*)" |grep -P -v "\\tchr" >' + tmpdir + '/' + tag + '.sam'
            subprocess.call(cmd, shell=True)
            print cmd
        if not ispe and fastq2 == '':
            cmd = b2url + ' ' + ' --maxins 450 --no-mixed --no-discordant --no-unal -p ' + str(nthreads) + ' -D 20 -R 3 -N 0 -L 20 -i S,1,0.50 -x ' + artindex + '/ucsc.artHLA -U ' + fastq1 + '|grep -P "\\t(A\\*|B\\*|C\\*|DQA1\\*|DQB1\\*|DRB1\\*)" |grep -P -v "\\tchr" > ' + tmpdir + '/' + tag + '.sam'
            subprocess.call(cmd, shell=True)
            print cmd
        if not ispe and fastq1 != '' and fastq2 != '':
            cmd = b2url + ' ' + ' --maxins 450 --no-mixed --no-discordant --no-unal -p ' + str(nthreads) + ' -D 20 -R 3 -N 0 -L 20 -i S,1,0.50 -x ' + artindex + '/ucsc.artHLA -U ' + fastq1 + '|grep -P "\\t(A\\*|B\\*|C\\*|DQA1\\*|DQB1\\*|DRB1\\*)" |grep -P -v "\\tchr" |awk  -F"\\t" \'BEGIN {OFS="\\t"} {$1=$1."_1"}{print $0}\' - >' + tmpdir + '/' + tag + '.tmp1.sam'
            subprocess.call(cmd, shell=True)
            cmd = b2url + ' ' + ' --maxins 450 --no-mixed --no-discordant --no-unal -p ' + str(nthreads) + ' -D 20 -R 3 -N 0 -L 20 -i S,1,0.50 -x ' + artindex + '/ucsc.artHLA -U ' + fastq2 + '|grep -P "\\t(A\\*|B\\*|C\\*|DQA1\\*|DQB1\\*|DRB1\\*)" |grep -P -v "\\tchr" |awk  -F"\\t" \'BEGIN {OFS="\\t"} {$1=$1."_2"}{print $0}\' - > ' + tmpdir + '/' + tag + '.tmp2.sam'
            subprocess.call(cmd, shell=True)
            print cmd
        ...
        cmd = 'samtools view -bS ' + tmpdir + '/' + tag + '.tmp.sam ' + ' > ' + tmpdir + '/' + tag + '.tmp.bam'
        print cmd
        subprocess.call(cmd, shell=True)
        cmd = 'samtools sort -n -o ' + tmpdir + '/' + tag + '_HLA.qnsorted.bam ' + tmpdir + '/' + tag + '.tmp.sam'
        print cmd
        subprocess.call(cmd, shell=True)
